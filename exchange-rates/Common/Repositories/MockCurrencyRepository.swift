...
final class MockCurrencyRepository: CurrencyRepository {
    ...
    func getLatestRates(baseCurrencyCode: String) async throws -> CurrencyExchangeResponse {
        ...
        guard let jsonData = getRateMockData() else {
            throw URLError(.cannotDecodeContentData)
        }
        return try JSONDecoder().decode(CurrencyExchangeResponse.self, from: jsonData)
    }
    
    func getCurrencyNames() async throws -> CurrencyNameResponse {
        try await Task.sleep(nanoseconds: delay)
        
        guard let jsonData = getNameMockData() else {
            throw URLError(.cannotDecodeContentData)
        }
        return try JSONDecoder().decode(CurrencyNameResponse.self, from: jsonData)
    }
}

extension MockCurrencyRepository {
    private func getRateMockData() -> Data? {
    ...
    private func getNameMockData() -> Data? {
        let string = """
        {
          "success": true,
          "symbols": {
